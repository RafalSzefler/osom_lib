...
impl PowerOfTwo64 {
    ...
    /// # Panics
    ///
    /// Panics in debug mode if the next power of two is greater than [`u64::MAX`].
    /// In release mode it wraps to zero.
    #[inline(always)]
    pub fn next(self) -> Self {
        debug_assert!(self.value < (u64::MAX >> 1));
    ...
    pub const fn as_usize(self) -> usize {
        ...
        {
            assert!(size_of::<usize>() >= size_of::<u64>(), "usize is smaller than u64");
            self.value as usize
        }
    }
}

impl core::fmt::Display for PowerOfTwo64 {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        self.value.fmt(f)
    }
}

impl TryClone for PowerOfTwo64 {
