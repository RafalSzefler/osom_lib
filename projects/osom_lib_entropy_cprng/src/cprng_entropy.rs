...
impl<TPrng> TryClone for CPRNGEntropy<TPrng>
where
    TPrng: PRNGenerator + CryptographicallySecure + Seedable<u128> + TryClone,
{
    type Error = <TPrng as TryClone>::Error;

    fn try_clone(&self) -> Result<Self, Self::Error> {
        let cprng = self.cprng.try_clone()?;
        Ok(Self { cprng })
    }
}
