    ...
    TConfig: BytellConfig + ReprC,
{
    const CHECK: () = const {
        ...
        osom_lib_reprc::hidden::is_reprc::<PowerOfTwo32>();
    ...
    TConfig: BytellConfig,
{
    ...
    pub fn with_capacity_and_config(capacity: Length, config: TConfig) -> Result<Self, HashTableError> {
        let number_of_items = capacity.as_u32();
        ...
        if expected_capacity >= u64::from(u32::MAX) {
    ...
    pub fn shrink_to_fit(&mut self) -> Result<(), HashTableError> {
        ...
        let new_config = unsafe { core::ptr::read(self.config.deref()) };
        let mut new_table = Self::with_block_count_and_config(new_blocks_count, new_config)?;
    ...
    fn with_block_count_and_config(block_count: PowerOfTwo32, config: TConfig) -> Result<Self, HashTableError> {
    ...
    pub(super) fn grow(&mut self) -> Result<(), HashTableError> {
        ...
        let new_config = unsafe { core::ptr::read(self.config.deref()) };
        let mut new_table = Self::with_block_count_and_config(new_block_count, new_config)?;
        self.move_content_to(&mut new_table);
        ...
        unsafe {
            ...
            while el_idx < capacity {
                ...
                *control_byte = ControlByte::EMPTY;
                ...
                let (key, value) = entry.kvp().read().unpack();

                other.insert_or_update_with(key, || value, |_| panic!("Update should not happen"));
    ...
    pub(super) fn clone_content_to(&self, other: &mut Self) -> Result<(), TryCloneHashTableError>
    where
        TKey: TryClone,
        TValue: TryClone,
    {
    ...
    pub(super) unsafe fn get_key_value_raw<Q>(&self, key: &Q) -> Option<*mut KVP<TKey, TValue>>
    where
        TKey: Borrow<Q>,
        Q: Eq + Hash + ?Sized,
    {
...
impl<TKey, TValue, TConfig> TryClone for BytellHashTable<TKey, TValue, TConfig>
where
    TKey: Eq + Hash + TryClone,
    TValue: TryClone,
    TConfig: BytellConfig + TryClone,
{
    ...
    fn try_clone(&self) -> Result<Self, Self::Error> {
        ...
        let mut new_table = Self::with_capacity_and_config(self.elements_count, new_config)?;
...
{
    fn clone(&self) -> Self {
        self.try_clone().expect("[BytellHashTable::try_clone] failure")
    }
}
    ...
    TConfig: BytellConfig,
{
    fn eq(&self, other: &Self) -> bool {
        ...
        for kvp in self.iter() {
            let Some(other_value) = other.get(kvp.key) else {
                return false;
            };
            if other_value != kvp.value {
                return false;
            }
        }
        true
