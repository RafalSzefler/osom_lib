...
impl<'a, TKey: 'a, TValue: 'a, TConfig> Iterator for BytellMutableIter<'a, TKey, TValue, TConfig> {
    type Item = KVP<&'a TKey, &'a mut TValue>;
    ...
    TConfig: BytellConfig,
{
    ...
    fn try_insert_or_update_with<'a, FAdd, FUpdate>(
        &mut self,
        key: TKey,
        adder: FAdd,
        updater: FUpdate,
            ...
            unsafe {
                ...
                if control_byte.is_direct_hit() {
                    ...
                    *free_entry.control_byte() = ControlByte::NEW_TAIL;
                    ...
                    let new_kvp = KVP { key, value: adder() };
                    free_entry.kvp().write(new_kvp);
                    self.elements_count += 1;

                    return Ok(&mut ptr_to_mut!(free_entry.kvp()).value);
                } else {
                    if self.should_grow() {
                        self.grow()?;
                        continue 'start;
                    ...
                    *entry.control_byte() = ControlByte::NEW_DIRECT_HIT;
                    let kvp = KVP { key, value: adder() };
                    entry.kvp().write(kvp);
                    return Ok(&mut ptr_to_mut!(entry.kvp()).value);
                }
            }
        }
    }
