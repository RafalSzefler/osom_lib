...
use crate::errors::{ArrayError, ArrayIsEmptyError, ArrayTryCloneError};
    ...
    TAllocator: Allocator,
{
    ...
    /// Initializes new [`InternalArray`], just like [`InternalArray::with_capacity`],
    ...
    pub fn with_capacity(capacity: Length, mut allocator: TAllocator) -> Result<Self, ArrayError> {
        ...
        T: TryClone,
    {
        ...
        unsafe {
            ...
            for _ in 0..slice.len() {
                let clone = (&*slice_val)
                    .try_clone()
                    .map_err(|_| ArrayTryCloneError::ItemCloningError)?;
                current_end.write(clone);
        ...
        TAllocator: TryClone,
    {
        ...
        let mut new_array = Self::with_capacity(self.length, allocator)?;
        new_array.try_push_slice(self.as_slice())?;
        Ok(new_array)
    }
    ...
    {
        let allocator = self.allocator.try_clone().map_err(|_| ArrayError::AllocationError)?;
        let mut new_array = Self::with_capacity(self.capacity, allocator)?;
        new_array.try_push_slice(self.as_slice())?;
        Ok(new_array)
    }
}
