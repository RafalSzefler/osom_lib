...
unsafe impl<const TCAPACITY: usize, T, TAllocator> Send for InlineDynamicArray<TCAPACITY, T, TAllocator>
where
    T: Sized + Send,
...
unsafe impl<const TCAPACITY: usize, T, TAllocator> Sync for InlineDynamicArray<TCAPACITY, T, TAllocator>
where
    T: Sized + Sync,
...
impl<const TCAPACITY: usize, T, TAllocator> core::fmt::Debug for InlineDynamicArray<TCAPACITY, T, TAllocator>
where
    T: Sized,
    TAllocator: Allocator,
{
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("InlineDynamicArray")
            .field("TCAPACITY", &TCAPACITY)
            .field("internal", &"[INLINE_ARRAY_UNION]")
    ...
    TAllocator: Allocator,
{
    const CHECK: () = const {
        ...
        osom_lib_reprc::hidden::is_reprc::<InlineArrayUnion<TCAPACITY, T>>();
    };
}
    ...
    T: ReprC,
{
    const CHECK: () = const {
        osom_lib_reprc::hidden::is_reprc::<T>();
        osom_lib_reprc::hidden::is_reprc::<*mut T>();
        osom_lib_reprc::hidden::is_reprc::<ManuallyDrop<MaybeUninit<[T; TCAPACITY]>>>();
    };
}
    ...
    TAllocator: Allocator,
{
    ...
    ///
    /// For details see [`ArrayError`].
    pub fn with_capacity_and_allocator(capacity: Length, mut allocator: TAllocator) -> Result<Self, ArrayError> {
