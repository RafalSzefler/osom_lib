...
/// A weak reference to the underlying [`SharedString`].
///
...
///     let strong1 = StdSharedString::from_str_slice("foo")?;
...
impl<TAllocator: Allocator> WeakSharedString<TAllocator> {
    ...
    /// Returns the number of strong references to the [`WeakSharedString`].
    #[inline(always)]
    #[must_use]
    ...
    /// Returns the number of weak references to the [`WeakSharedString`].
    #[inline(always)]
    #[must_use]
    ...
    /// Upgrades current weak reference to the strong [`SharedString`].
    ///
    ///
    /// # Errors
    ///
    ...
    /// In particular only single (the last) [`WeakSharedString`] returns true
    /// by calling this.
    #[inline(always)]
    #[must_use]
