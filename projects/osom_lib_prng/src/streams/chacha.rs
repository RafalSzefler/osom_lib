...
impl<const ROUNDS: u32> ChaChaStream<ROUNDS> {
    ...
    pub fn from_slices_and_counter(key: &[u8], nonce: &[u8], counter: u32) -> Self {
        const {
            ...
            assert!(ROUNDS <= 1000, "ChaCha rounds must be at most 1000. That number is definetely too much for any purpose. The recommended value is 20.");
        }
        assert!(key.len() == 32, "ChaCha key must be of size 32");
        assert!(nonce.len() == 12, "ChaCha nonce must be of size 12");
        assert!(counter < u32::MAX, "ChaCha counter must be smaller than u32::MAX. It is recommended to make it 0 or 1 or something low. Otherwise overflow will occure fast.");
